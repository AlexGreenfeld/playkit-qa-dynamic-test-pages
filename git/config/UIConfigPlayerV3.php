...
//ini_set('display_errors', 1);
	...
	class Radiobutton{
		...
		public $label;
		public $name;
		public function  __construct($name,$val,$text) {
			$this->name = $name;
			$this->value = $val;
			$this->label = $text;
		}
	...
	class Textbox{
		...
		public $value;
		public $size;
		public function  __construct($name,$value,$size) {
			$this->name = $name;
			$this->value = $value;
			$this->size = $size;
		}
...
//	$uiConfId_test_ovp_cc =    new Option("testOvpCC","limeGreen","15217680 - Testing OVP CC");
...
//	$uiConfId_test_drm_cc =    new Option("testDrmCC","green","15217867 - Testing DRM CC");
...
//	$uiConfId_test_ott_cc =    new Option("testOttCC","purple","15217731 - Testing OTT CC");
...
//	$uiConfId_ott_prod =       new Option("ottProd","purple","42700261 - OTT production");
	$uiConfIds =               array($uiConfId_test, $uiConfId_test_latest, $uiConfId_no_plug_test, $uiConfId_test_youbora, $uiConfId_test_drm, $uiConfId_prod, $uiConfId_prod_latest, $uiConfId_prod_drm, $uiConfId_ott_test, $uiConfId_no_plug_ott);
...
//	$entryId_testDrm360 =      new Option("tstDrm360","black","0_nq4v8mc2 - DRM 360 (no supported)");
	...
	$entriesId =  array($entryId_empty, $entryId_testBasic, $entryId_testBP, $entryId_testLong, $entryId_testNoCap, $entryId_testABR, $entryId_testManSwitch, $entryId_testCapAud, $entryId_testEngCapDef, $entryId_testCapDef, $entryId_testCapCaption, $entryId_testMultiCaption, $entryId_testAud, $entryId_testAudEnt, $entryId_testAudEnt2, $entryId_testDrm, $entryId_testFair, $entryId_testDrmAbr, $entryId_prodBasic, $entryId_prodCap, $entryId_prodAud, $entryId_prodDrm, $entryId_prodFairPlay, $entryId_testKltLive, $entryId_testKltLiveDvr, $entryId_testUniLive, $entryId_testUniLiveDvr, $entryId_testExtLive, $entryId_testExtLiveCapt, $entryId_testWnIntSrcLive, $entryId_testCarbonLive, $entryId_testExtLiveAbc, $entryId_test1TVRusLive, $entryId_testMatchTv, $entryId_prodKltLiveDvr, $entryId_prodKltLiveDrm, $entryId_10SecQrMidRight, $entryId_30SecQrMidRight, $entryId_30SecQr, $entryId_60SecQr, $entryId_ottLongVod, $entryId_ottVod, $entryId_ottExtLive, $entryId_ottDrm, $entryId_testExoplan360, $entryId_testAirbus360, $entryId_testStarWars360, $entryId_testWild360, $entryId_testSkyDive360, $entryId_prod360);
...
*/
	...
	//**************** Log level **************************************************************************************
	$chkTrace = 			 new Radiobutton("logs","trace","Trace");
	$chkDebug = 			 new Radiobutton("logs","deb","Debug");
	$chkInfo = 			  	 new Radiobutton("logs","inf","Info");
	$chkWarning = 			 new Radiobutton("logs","warn","Warning");
	$chkError = 			 new Radiobutton("logs","err","Error");
	...
	$chkNewPlayerConf = 	   new Checkbox("newConf","Load with old player configuration structure");
	$chkPreLoad = 			   new Checkbox("preEnb","Preload");
	$chkLoopBack = 			   new Checkbox("loopEnb","Loopback");
	...
	$midPost =          	   new Option("midPost","black","Mid-Post rolls with skip");
	$advertisiments =          array($noAd, $emptyAdTag, $VAMPAdsPlaylistWithSkip, $preWithoutSkip, $preWithSkip, $redirectLinear, $redirectError, $redirectBrokFall, $nonLinear, $vmapPreroll, $vmapPrerollBumper, $vmapPostroll, $vmapPostrollBumper, $vmapPreMidPost, $vmapPreMidStPost, $vmapPreMidOpPost, $vmapPreMidStPostBumper, $vmapPreMidOpPostBumper, $vmapPre10MidPost, $preCompanions, $midPost);
	...
	$chkCC =          new Checkbox("ccEnb","ChromeCast Sender");
  ...
  $automationOptions = array($automationDefault,$automation1);	
	...
	$chkDisableUi =       new Checkbox("DisUI","Disable UI");

	//****************** Change Media *****************************************************************************
	$chkChangeMedia =     new Checkbox("chgMedia","Enable");
	...
	function checkBoxChecked($chkName){
		...
		$chkText = '';
			if ( $_GET[$chkName->value]== 'on'){
			 ...
			 $checkBoxString = $checkBoxString . '<input ' .$chkText. ' type="checkbox" id="' .$chkName->value. '" name="' .$chkName->value. '">' .$chkName->label. ' ';
	...
	function textBoxMaker($txtBoxName){
		$textBoxString = '';
		$textBoxString = $textBoxString . '<input type="text" id="'.$txtBoxName->name.'" name="'.$txtBoxName->name.'" size="'.$txtBoxName->size.'" value="'.$txtBoxName->value.'">';
	...
	function radioButtonChecked($chkName){
		...
		if ( $_GET[$chkName->name] == $chkName->value){
			 ...
			 $radioButtonString = $radioButtonString . '<input ' .$chkText. ' type="radio" id ="' .$chkName->value. '" value="' .$chkName->value. '" name="' .$chkName->name. '">' .$chkName->label. ' ';
	...
	<script type="text/javascript">
		...
		function pluginsVersions(){
			...
			for (i =0; i < pluginsDict.length; i++){
    			plugins = plugins + (pluginsDict[i].key + " - " + pluginsDict[i].value)+ " ; ";
			}
			
			return document.getElementById("pluginsVer").textContent = "Plugins versions:  \n\n"  + plugins;
		}
		...
		function getSelectValue(sel_tag_id,input_tag_id,first_char=0,last_char){
...
<?php
	echo '
	<body onload="getSelectValue(\'chkEntry\',\'selectedEntryId\',0,10); getSelectValue(\'chkUiConf\',\'selectedUiConf\',0,8); pluginsVersions()" >
	<div id="menuButton">
	...
	<div>
		...
		' . str_repeat("&nbsp;", 8) . '<p id="playerVersion" style="color:blue; font-weight:bold; display:inline"></p>
		'. str_repeat("&nbsp;", 8) .'<h6 style="font-weight:bold; color:green; display:inline"> Delivery type </h6>
		<input type="text" id="curDelType" size="3" style="font-weight:bold; color:green; display:inline; text-align:center" disabled />
		<p id="pluginsVer" style="font-weight:bold;"></p>
	</div>
			...
			<tr>
			  ...
			  <tr>				  
				<td class="tg-text"> UIConf ID &nbsp;&nbsp
					<select id="chkUiConf" name="uiConfId" onchange="getSelectValue(\'chkUiConf\',\'selectedUiConf\',0,8)">'
					  . optionsMakerWithSelected($uiConfIds,'uiConfId') .
					'</select>
					<input type="text" id="selectedUiConf" size="8" readonly>
			    </td>
			  </tr>	
			  
			  <tr>				  
				<td class="tg-text"> Entry ID &nbsp;&nbsp
					<select id="chkEntry" name="entryId" onchange="getSelectValue(\'chkEntry\',\'selectedEntryId\',0,10)">'
					  . optionsMakerWithSelected($entriesId,'entryId') .
					'</select>
					<input type="text" id="selectedEntryId" size="8"  readonly>
				</td>
			  ...
			  <tr>				  
				<td class="tg-text"> Change Media &nbsp;&nbsp
					<select id="chkChgMed" name="chgMed" >'
					  . optionsMakerWithSelected($entriesId,'chgMed') .
					'</select>
					'
						 .	checkBoxChecked($chkChangeMedia) .
					'</td>
